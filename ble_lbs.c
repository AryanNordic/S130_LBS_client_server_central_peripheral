...
static void on_write(ble_lbs_t * p_lbs, ble_evt_t * p_ble_evt)
{
    ...
    {
        p_lbs->led_write_handler(p_evt_write->data[0], p_ble_evt->header.evt_id);
    }
}
...
void ble_lbs_on_ble_evt(ble_lbs_t * p_lbs, ble_evt_t * p_ble_evt)
{
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GATTC_EVT_HVX:
            // client_handler will handle this
            break;            
 ...
 *
 */
static uint32_t button_notif_char_add(ble_lbs_t * p_lbs)
{
    ble_gatts_char_md_t char_md;
...
uint32_t ble_lbs_init(ble_lbs_t * p_lbs)
{
    ...
    err_code = button_notif_char_add(p_lbs);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t ble_lbs_write_button_change(ble_lbs_t * p_lbs, uint8_t button_state)
{
    return client_handling_write_char_lbs(p_lbs->conn_handle, button_state);
}

uint32_t ble_lbs_notif_button_change(ble_lbs_t * p_lbs, uint8_t button_state)
{
